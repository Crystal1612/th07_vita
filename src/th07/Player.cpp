...
extern i32 TH_BUTTON_SHOOT_MAP[CONTROL_RECEIVER];// = {0,TH_BUTTON_SHOOT,TH_BUTTON_SHOOT2,TH_BUTTON_SHOOT3};
extern i32 TH_BUTTON_BOMB_MAP[CONTROL_RECEIVER];// = {0,TH_BUTTON_BOMB,TH_BUTTON_BOMB2,TH_BUTTON_BOMB3};
extern i32 TH_BUTTON_FOCUS_MAP[CONTROL_RECEIVER];// = {0,TH_BUTTON_FOCUS,TH_BUTTON_FOCUS2,TH_BUTTON_FOCUS3};
extern i32 TH_BUTTON_UP_MAP[CONTROL_RECEIVER];// = {0,TH_BUTTON_UP,TH_BUTTON_UP2,TH_BUTTON_UP3};
extern i32 TH_BUTTON_DOWN_MAP[CONTROL_RECEIVER];// = {0,TH_BUTTON_DOWN,TH_BUTTON_DOWN2,TH_BUTTON_DOWN3};
extern i32 TH_BUTTON_LEFT_MAP[CONTROL_RECEIVER];// = {0,TH_BUTTON_LEFT,TH_BUTTON_LEFT2,TH_BUTTON_LEFT3};
extern i32 TH_BUTTON_RIGHT_MAP[CONTROL_RECEIVER];// = {0,TH_BUTTON_RIGHT,TH_BUTTON_RIGHT2,TH_BUTTON_RIGHT3};
                               ...
                               ShtEntry *shtEntry)
{
    ...
    *bullet->GetPosY() += shtEntry->offset.y;
    ...
    g_AnmManager->SetAnmIdxAndExecuteScript(&bullet->vm, shtEntry->anmFileIdx+player->anmOffsetPlayer);
}
...
i32 Player::HandlePlayerInputs()
{
    ...
    if (IS_PRESSED_GAME(TH_BUTTON_UP_MAP[this->playerType]))
    {
        this->playerDirection = MOVEMENT_UP;
        if (IS_PRESSED_GAME(TH_BUTTON_LEFT_MAP[this->playerType]))
        {
            this->playerDirection = MOVEMENT_UP_LEFT;
        }
        if (IS_PRESSED_GAME(TH_BUTTON_RIGHT_MAP[this->playerType]))
        {
            this->playerDirection = MOVEMENT_UP_RIGHT;
        }
    }
    else if (IS_PRESSED_GAME(TH_BUTTON_DOWN_MAP[this->playerType]))
    {
    ...
    if (IS_PRESSED_GAME(TH_BUTTON_FOCUS_MAP[this->playerType]))
    {
        this->isFocus = 1;
    ...
    {
        g_AnmManager->SetAnmIdxAndExecuteScript(&this->playerSprite, ANM_SCRIPT_PLAYER_STOPPING_LEFT + this->anmOffsetPlayer);
    }

    if (horizontalSpeed > 0.0f && this->previousHorizontalSpeed <= 0.0f)
    {
        g_AnmManager->SetAnmIdxAndExecuteScript(&this->playerSprite, ANM_SCRIPT_PLAYER_MOVING_RIGHT + this->anmOffsetPlayer);
    }
    else if (horizontalSpeed == 0.0f && this->previousHorizontalSpeed > 0.0f)
    {
        g_AnmManager->SetAnmIdxAndExecuteScript(&this->playerSprite, ANM_SCRIPT_PLAYER_STOPPING_RIGHT + this->anmOffsetPlayer);
    }
    ...
    *GetPosCenterY() += this->velocity.y;
    ...
    if (this->character != CHAR_SAKUYA || this->shotType != 1)
    {
    ...
    if (IS_PRESSED_GAME(TH_BUTTON_SHOOT_MAP[this->playerType]) && !g_Gui.HasCurrentMsgIdx())
    {
...
void Player::UpdateBorderAndBombState()
{
    if (this->hasBorder != BORDER_NONE && !this->bombInfo.isInUse &&
        IS_PRESSED_GAME(TH_BUTTON_BOMB_MAP[this->playerType]))
    {
        BreakBorder(1);
        this->isBombing = 0;
    ...
    else
    {
        ...
        if (this->invulnerabilityTimer.GetCurrent() >= 30)
        {
            ...
            g_AnmManager->SetAnmIdxAndExecuteScript(&this->playerSprite,
                                                    ANM_SCRIPT_PLAYER_IDLE + this->anmOffsetPlayer);
            if ((i32)g_GameManager.globals->livesRemaining <= 0)
            {
...
void Player::BreakBorder(u32 unused)
{
    ...
    {
        effect = g_EffectManager.SpawnEffect(29, &this->positionCenter, 1,
                                             0xffffffff);
...
ZunResult Player::AddedCallback(Player *arg)
{
    ...
    if (ShtData::LoadShtData(
            &arg->shooterData,
            g_ShooterTable[arg->shotTypeAndCharacter]) != ZUN_SUCCESS)
    {
        return ZUN_ERROR;
    }
    ...
    if (ShtData::LoadShtData(
            ...
            g_ShooterTableFocus[arg->shotTypeAndCharacter]) !=
        ZUN_SUCCESS)
    {
        return ZUN_ERROR;
    }
    ...
    g_AnmManager->SetAnmIdxAndExecuteScript(&arg->optionsSprite[0], ANM_SCRIPT_PLAYER_OPTION_LEFT + arg->anmOffsetPlayer);
    g_AnmManager->SetAnmIdxAndExecuteScript(&arg->optionsSprite[1], ANM_SCRIPT_PLAYER_OPTION_RIGHT + arg->anmOffsetPlayer);
    bullet = arg->bullets;
    ...
    arg->bombInfo.bombCalc =
        g_BombData[arg->shotTypeAndCharacter].calc;
    arg->bombInfo.draw = g_BombData[arg->shotTypeAndCharacter].draw;
    arg->bombInfo.bombFocusCalc =
        g_BombData[arg->shotTypeAndCharacter].calcFocus;
    arg->bombInfo.drawFocus =
        g_BombData[arg->shotTypeAndCharacter].drawFocus;
...
ZunResult Player::DeletedCallback(Player *arg)
{
    if ((u32)(g_Supervisor.curState != SUPERVISOR_STATE_NEXT_STAGE &&
              ...
              g_Supervisor.curState != SUPERVISOR_STATE_NEXT_STAGE_USELESS))
    {
        g_AnmManager->ReleaseAnm(ANM_FILE_PLAYER+arg->anmFilePlayer);
...
ZunResult Player::RegisteringChain(Player *mgr, u32 param_1, u8 playerType){
    memset(mgr, 0, sizeof(Player));
    mgr->playerType = playerType;
...
{
    if(RegisteringChain(&g_Player,param_1,1)==ZUN_ERROR){
        return ZUN_ERROR;
    }
    // apply it's controller. compatslop
    g_Player.playerEffectAttach = 24;
    
    if(RegisteringChain(&g_Player2,param_1,2)==ZUN_ERROR){
        return ZUN_ERROR;
    }
...
ZunResult ShtData::LoadShtData(ShtData **data, const char *shtPath)
{
    ...
    *data = (ShtData *)FileSystem::OpenFile(shtPath, 0);
    ...
    for (i = 0; i < (i32)(u32)(*data)->numLevels; i++)
    {
        (*data)->levels[i].entry =
