...
{
    this->impl->statusPopup.pos = ZunVec3(416.0f, 168.0f, 0.0f);
...
void Gui::CopyEnemyNameTexture(i32 spriteIdx)
{
    ...
    dstRect.x = g_AnmManager->GetSprite(0x609)->startPixelInclusive.x;
    dstRect.y = g_AnmManager->GetSprite(0x609)->startPixelInclusive.y;
    dstRect.w = g_AnmManager->GetSprite(0x609)->endPixelInclusive.x - dstRect.x;
    dstRect.h = g_AnmManager->GetSprite(0x609)->endPixelInclusive.y - dstRect.y;
...
u32 Gui::OnDraw(Gui *arg)
{
    ...
    case GUI_DISPLAY_FULL_POWER:
        ...
        AsciiManager::AddFormatText(&g_AsciiManager, &arg->impl->statusPopup.pos,
                                    "Full Power Mode!");
        g_AsciiManager.color = 0xffffffff;
        break;
    ...
    case GUI_DISPLAY_BORDER_BONUS:
        ...
        g_AsciiManager.color = 0xffe0b0ff;
        AsciiManager::AddFormatText(&g_AsciiManager, &arg->impl->statusPopup.pos,
                                    "Border Bonus %7d", arg->impl->statusPopup.fmtArg);
        g_AsciiManager.color = 0xffffffff;
        g_AsciiManager.scale.x = 1.0f;
        g_AsciiManager.scale.y = 1.0f;
        g_AsciiManager.fontSpacing = 14;
